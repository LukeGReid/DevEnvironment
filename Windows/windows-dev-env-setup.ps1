...
(
    ...
    [Switch]$FeshKeys, #Removes existing SSH keys and the script will request that you re-generate them again
    [string]$GitServiceDNS = "github",
    [string]$GitServiceURL = "https://$dns_GitServer"
)
$RegularInstall = $true
...
####### Variables ########
#URLS
$dns_GitServer = $GitServiceDNS
$url_GitServer = $GitServiceURL
...
$VSCode_configurationList = @{
    "gitlab.instanceUrl" = $url_GitServer
...
if($CleanInstall)
{
    ...
    {
        Write-Warning "Removing $dns_GitServer from SSH known_hosts"
        Set-Content -Path $path_SSHKnownHosts -Value (get-content -Path $path_SSHKnownHosts | Select-String -Pattern $dns_GitServer -NotMatch)
...
if($FeshKeys)
{
    Write-Warning "Removing $dns_GitServer from Git known_hosts"
    Set-Content -Path $path_SSHKnownHosts -Value (get-content -Path $path_SSHKnownHosts | Select-String -Pattern $dns_GitServer -NotMatch)
...
if($UserProfileOnly -or $RegularInstall)
{
    ...
    Write-Host "Please confirm your SSH version! The version of SSH should be greater than 8.2. The current version is:"  -ForegroundColor Cyan -BackgroundColor Black
    ssh -V
    Write-Host "If the version isn't greater than 8.2 you may still have to remove the default version of SSH installed by windows."  -ForegroundColor Cyan -BackgroundColor Black
    ...
    if(!(Test-Path $path_SSHKnownHosts) -or ((Get-Content $path_SSHKnownHosts | Select-String -Pattern $dns_GitServer).Length -eq 0))
    {
        ssh-keyscan -t $configSSH_KeyType $dns_GitServer >> $path_SSHKnownHosts
    }
    Write-Host "Please make sure that you store the passphrase some safe like a password manager. Copy the keys created at $path_SSHSettingsDirectory into an SSH Key secret type." -ForegroundColor Cyan -BackgroundColor Black
    ...
    #Generate warnings
    Write-Host "Copy the public key $path_SSHSettingsDirectory\id_$configSSH_KeyType.pub to the SSH keys on your $url_GitServer account." -ForegroundColor Cyan -BackgroundColor Black
    Start-Process $path_SSHSettingsDirectory
    New-Item -Path $path_SSHSettingsDirectory -Name "_ PLEASE OPEN THE id_$configSSH_KeyType.pub FILE AND COPY THE WHOLE LINE CONTAINING $dns_GitServer INTO $dns_GitServer" -ItemType "File" -Value "" -Force | Out-Null
}
